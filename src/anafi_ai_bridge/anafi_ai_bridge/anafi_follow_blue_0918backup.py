...
class CreativeBehaviorFollowBlue(Node):
    """
    파란 LED(HSV) 추적 → PilotingCommand 퍼블리시
    - 입력: /<ns>/camera/image (sensor_msgs/Image, bgr8)
    - 출력: /<ns>/drone/command (anafi_ros_interfaces/PilotingCommand)
    """
    def __init__(self):
    ...
    def on_image(self, msg: Image):
        # BGR 이미지 획득
        try:
            cv_img = self.bridge.cv2_to_imgmsg  # 실수 방지용 속성 체크
        except Exception:
            pass
    ...
    def control_loop(self):
        ...
        if lost or self.last_cx_norm is None or self.last_diam_px is None:
            ...
            # 추가로 살짝 상수 전진을 원하면 pitch_deg += 상수
