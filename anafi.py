...
class Anafi(Node):
    ...
    def __init__(self):
        ...
        if self.skycontroller_enabled:  # connect to SkyController
            print('connected using the skycontroller')
        ...
        ## streaming related things are here
        ...
        self.drone.streaming.set_output_files(
            video=os.path.join("streaming.mp4"),
            metadata=os.path.join("streaming_metadata.json"),
        )
        ...
        # print('PDRAW Status: ', self.drone.streaming._state)
        # print('PDRAW State Printed 469')
        # try:
        #     # self.drone.streaming.stop()      
        #     print("Setting the output file")  
        ...
        # print('Start Streaming at Media Name: ', media_name)
    ...
    def _streaming_start(self, media_name: str):
        """Playing 중복 방지: 항상 stop 후 짧은 간격을 두고 start"""
    ...
    def _streaming_watchdog(self):
        ...
        if now - last > 1.0:
            ...
            media_name = "Front camera" if self.model == 'ai' else "DefaultVideo"
            # media_name = "Disparity map"
            try:
                self._streaming_start(media_name)
                self._last_frame_time = time.time()
            except Exception as e:
                self.node.get_logger().error(f"streaming restart failed: {e}")
    ...
    def connect(self):
        ...
        while True:  # connection loop
            ...
            if self.skycontroller_enabled or self.drone_serial == "": # 이 path로 connection이 생기는 것으로 보임
                print("Streaming Connected Using Standar API")
    ...
    def parameter_callback(self, parameters):	
        for parameter in parameters:
            ...
            # if parameter.name == 'camera/stereo/disparity_map' and self.model in {'ai'}:
            #     self.disparity_map = parameter.value
            #     target_media = ("Disparity map" if self.disparity_map else "Front camera")
            #     self._streaming_start(target_media)
            #     self.node.get_logger().debug("Parameter 'camera/stereo/disparity_map' set to %r" % self.disparity_map)
    ...
    def yuv_frame_cb(self, yuv_frame):  # this function will be called by Olympe for each decoded YUV frame
        ...
        #     if dt > 0.04:
        #         print(f"[WARN] Decoder lag detected")
    ...
    def yuv_frame_processing(self):
        ...
        while rclpy.ok():
            ...
            print(f"Frame Got at time {time.time()}")
            ...
            try:
                ...
                if ts is not None:
                    ...
                    self.node.get_logger().warn("NTP timestamp missing, Using ROS clock.")
    ...
    def land_callback(self, request, response):
        ...
        response.success = True
        response.message = "Landing command sent"
        return response
...
def main(args=None):
    ...
    #     pass
    # finally:
    #     print("Stopping streaming and saving video...")
    #     anafi.drone.streaming.stop()

    #     time.sleep(5) 
        
    #     anafi.destroy_node()
    #     rclpy.shutdown()
    # rclpy.spin(anafi.node)2
